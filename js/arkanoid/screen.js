Arkanoid.Button = Arkanoid.Komponent.extend({
	onClick: $.noop,
...
Arkanoid.Screen = Base.extend({
    ...
    constructor: function (paper) {
		...
		var self = this;
        this.ball = new Arkanoid.Ball(this, this.paper);
        this.bat = new Arkanoid.Bat(this.paper);
        this.scoreCounter = new Arkanoid.ScoreCounter(this.paper);
		this.livesCounter = new Arkanoid.LivesCounter(this.paper);
		this.livesCounter.onNoLifeLeft = function() {
			self.gameOver();
		}
    },
    init: function (keepScore) {
		this.screen = Arkanoid.screens[this.level];
        // grid...
    ...
    putStartButton: function (keepScore) {
		var self = this;
		var btn = new Arkanoid.Button(this.paper, "Start!", "orange");
    ...
    putWellDoneButton: function () {
		var self = this;
		var btn = new Arkanoid.Button(this.paper, "Well done!", "cyan");
        btn.onClick = function () {
			self.level++;
			if(self.level >= Arkanoid.screens.length) self.level = 0;
            self.init(true);
        };
        return btn;
    },
	...
	gameOver: function() {
		var self = this;
		self.endGame();
		var btn = new Arkanoid.Button(this.paper, "Game over!", "#888");
        btn.onClick = function () {
			self.level = 0;
			self.init();
        };
    ...
    putBrick: function (type, x, y) {
        var self = this;
        var ab = new Arkanoid.Brick(this.paper, type, x, y);
